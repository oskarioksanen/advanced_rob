...
class Reactive_Controller : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        if (ctr_obj_ == 1)
        {
            if (!n.getParam("/elfin/Reactive_Controller/clik_gain/K_regulation", K_regulation_))
            {
        ...
        {
            if (!n.getParam("/elfin/Reactive_Controller/clik_gain/K_tracking", K_tracking_))
            {
        ...
        // 6.2 subsriber
        sub_x_cmd_ = n.subscribe<std_msgs::Float64MultiArray>("/trajectory_topic", 1, &Reactive_Controller::commandCB, this);
    ...
    void update(const ros::Time &time, const ros::Duration &period)
    {
        ...
        ROS_INFO("x_cmd_ here: %f , %f , %f", (double)x_cmd_(0), (double)x_cmd_(1), (double)x_cmd_(2));
        xd_.p(0) = x_cmd_(0);
        xd_.p(1) = x_cmd_(1);
        xd_.p(2) = x_cmd_(2);
