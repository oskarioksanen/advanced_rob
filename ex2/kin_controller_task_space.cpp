...
{
class Kin_Controller_Task_Space : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        for (size_t i = 0; i < n_joints_; i++)
        {
            ...
            if (n.getParam("/elfin/Kin_Controller_Task_Space/gains/elfin_joint" + si + "/pid/p", Kp[i]))
            {
                Kp_(i) = Kp[i];
            }
            else
            {
                std::cout << "/elfin/Kin_Controller_Task_Space/gains/elfin_joint" + si + "/pid/p" << std::endl;
                ROS_ERROR("Cannot find pid/p gain");
                return false;
            }

            if (n.getParam("/elfin/Kin_Controller_Task_Space/gains/elfin_joint" + si + "/pid/i", Ki[i]))
            {
                Ki_(i) = Ki[i];
            }
            else
            ...
            if (n.getParam("/elfin/Kin_Controller_Task_Space/gains/elfin_joint" + si + "/pid/d", Kd[i]))
            {
                Kd_(i) = Kd[i];
            }
            else
    ...
    void update(const ros::Time &time, const ros::Duration &period)
    {
        ...
        //e_cmd_j=V_cmd_-J_.data*q_dot_.data;
        ...
        //aux_d_.data = M_.data * (q_dotdot_cmd_.data + Kd_.data.cwiseProduct(V_cmd_-J_.data*qdot_.data));
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::Kin_Controller_Task_Space, controller_interface::ControllerBase)
