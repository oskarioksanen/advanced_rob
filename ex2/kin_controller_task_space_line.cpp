...
{
class Kin_Controller_Task_Space_Line : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        for (size_t i = 0; i < n_joints_; i++)
        {
            ...
            if (n.getParam("/elfin/Kin_Controller_Task_Space_Line/gains/elfin_joint" + si + "/pid/p", Kp[i]))
            {
                Kp_(i) = Kp[i];
            }
            else
            {
                std::cout << "/elfin/Kin_Controller_Task_Space_Line/gains/elfin_joint" + si + "/pid/p" << std::endl;
                ROS_ERROR("Cannot find pid/p gain");
                return false;
            }

            if (n.getParam("/elfin/Kin_Controller_Task_Space_Line/gains/elfin_joint" + si + "/pid/i", Ki[i]))
            {
                Ki_(i) = Ki[i];
            }
            else
            ...
            if (n.getParam("/elfin/Kin_Controller_Task_Space_Line/gains/elfin_joint" + si + "/pid/d", Kd[i]))
            {
                Kd_(i) = Kd[i];
            }
            else
    ...
    void update(const ros::Time &time, const ros::Duration &period)
    {
        ...
        {
        //Own code:
            qd_(i) = (1-tau_k)*q_0_(i)+tau_k*q_1_(i);
            qd_dot_(i) = v_tau*(q_1_(i)-q_0_(i));
            qd_ddot_(i) = 0;
            //Straight line trajectory in joint space:
        ...
        V_cmd_ = Vd_ + Kp_.data.cwiseProduct(xerr_);
        //Testi:
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::Kin_Controller_Task_Space_Line, controller_interface::ControllerBase)
