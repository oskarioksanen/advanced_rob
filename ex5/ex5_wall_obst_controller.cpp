...
{
class Wall_obst_controller : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        for (size_t i = 0; i < n_joints_; i++)
        {
            ...
            if (n.getParam("/elfin/Wall_obst_controller/gains/elfin_joint" + si + "/pid/p", Kp[i]))
            {
                Kp_(i) = Kp[i];
            }
            else
            {
                std::cout << "/elfin/Wall_obst_controller/gains/elfin_joint" + si + "/pid/p" << std::endl;
                ROS_ERROR("Cannot find pid/p gain");
                return false;
            }

            if (n.getParam("/elfin/Wall_obst_controller/gains/elfin_joint" + si + "/pid/i", Ki[i]))
            {
                Ki_(i) = Ki[i];
            }
            else
            ...
            if (n.getParam("/elfin/Wall_obst_controller/gains/elfin_joint" + si + "/pid/d", Kd[i]))
            {
                Kd_(i) = Kd[i];
            }
            else
        ...
        sub_keyboard_input_ = n.subscribe<std_msgs::String>("/ex5_keyboard_input_topic", 1, &Wall_obst_controller::keyboardCommandCB, this);
        ...
        //Koitetaan saada point2 vähän kauemmas point1:stä, jotta saadaan obstacle järkevämmin väliin:
        /*point2_.p(0) = point1_.p(0)-0.1;
        point2_.p(1) = point1_.p(1);
        point2_.p(2) = point1_.p(2)+0.05;
        point2_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));*/
        
        //Muista päivittää nämä jos muuttaa wallin kokoa tai posea
        obst_size_x = 0.01;
    	obst_size_y = 0.07;
    	obst_size_z = 0.1;
    	/*obst_origin_.p(0) = -0.1;
        obst_origin_.p(1) = -0.4;
        obst_origin_.p(2) = 0.7;*/
        //Koitetaan palauttaa vanhat:
        obst_origin_.p(0) = -0.25;
        obst_origin_.p(1) = -0.1;
        obst_origin_.p(2) = 0.45;
        obst_origin_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        
        point1_.p(0) = obst_origin_.p(0)+0.15;
        point1_.p(1) = obst_origin_.p(1);
        point1_.p(2) = obst_origin_.p(2);
        point1_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        
        point2_.p(0) = obst_origin_.p(0)-0.18;
        point2_.p(1) = obst_origin_.p(1);
        point2_.p(2) = obst_origin_.p(2)+0.05;
        point2_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        
        obst_top_right_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_top_right_front_.p(1)=obst_origin_.p(1)+0.5*obst_size_y;
        obst_top_right_front_.p(2)=obst_origin_.p(2)+0.5*obst_size_z;
        obst_top_right_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_top_right_front_);
        
        obst_top_middle_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_top_middle_front_.p(1)=obst_origin_.p(1);
        obst_top_middle_front_.p(2)=obst_origin_.p(2)+0.5*obst_size_z;
        obst_top_middle_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_top_middle_front_);
        
        obst_top_left_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_top_left_front_.p(1)=obst_origin_.p(1)-0.5*obst_size_y;
        obst_top_left_front_.p(2)=obst_origin_.p(2)+0.5*obst_size_z;
        obst_top_left_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_top_left_front_);
        
        obst_top_right_back_.p(0)=obst_origin_.p(0)-0.5*obst_size_x;
        obst_top_right_back_.p(1)=obst_origin_.p(1)+0.5*obst_size_y;
        obst_top_right_back_.p(2)=obst_origin_.p(2)+0.5*obst_size_z;
        obst_top_right_back_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_top_right_back_);
        
        obst_top_middle_back_.p(0)=obst_origin_.p(0)-0.5*obst_size_x;
        obst_top_middle_back_.p(1)=obst_origin_.p(1);
        obst_top_middle_back_.p(2)=obst_origin_.p(2)+0.5*obst_size_z;
        obst_top_middle_back_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_top_middle_back_);
        
        obst_top_left_back_.p(0)=obst_origin_.p(0)-0.5*obst_size_x;
        obst_top_left_back_.p(1)=obst_origin_.p(1)-0.5*obst_size_y;
        obst_top_left_back_.p(2)=obst_origin_.p(2)+0.5*obst_size_z;
        obst_top_left_back_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_top_left_back_);
        
        obst_bottom_right_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_bottom_right_front_.p(1)=obst_origin_.p(1)+0.5*obst_size_y;
        obst_bottom_right_front_.p(2)=obst_origin_.p(2)-0.5*obst_size_z;
        obst_bottom_right_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_bottom_right_front_);
        
        obst_bottom_middle_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_bottom_middle_front_.p(1)=obst_origin_.p(1);
        obst_bottom_middle_front_.p(2)=obst_origin_.p(2)-0.5*obst_size_z;
        obst_bottom_middle_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_bottom_middle_front_);
        
        obst_bottom_left_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_bottom_left_front_.p(1)=obst_origin_.p(1)-0.5*obst_size_y;
        obst_bottom_left_front_.p(2)=obst_origin_.p(2)-0.5*obst_size_z;
        obst_bottom_left_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_bottom_left_front_);
        
        obst_middle_right_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_middle_right_front_.p(1)=obst_origin_.p(1)+0.5*obst_size_y;
        obst_middle_right_front_.p(2)=obst_origin_.p(2);
        obst_middle_right_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_middle_right_front_);
        
        obst_middle_middle_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_middle_middle_front_.p(1)=obst_origin_.p(1);
        obst_middle_middle_front_.p(2)=obst_origin_.p(2);
        obst_middle_middle_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_middle_middle_front_);
        
        obst_middle_left_front_.p(0)=obst_origin_.p(0)+0.5*obst_size_x;
        obst_middle_left_front_.p(1)=obst_origin_.p(1)-0.5*obst_size_y;
        obst_middle_left_front_.p(2)=obst_origin_.p(2);
        obst_middle_left_front_.M = KDL::Rotation(KDL::Rotation::RPY(0, 0, 0));
        obst_points_.push_back(obst_middle_left_front_);
    ...
    {
    	//Eigen::Matrix<double, num_taskspace, 1>rep_velocities_task_space;
	//rep_velocities_task_space.data = Eigen::VectorXd::Zero(n_joints_);
	int k = 2;
	double q_star = 0.1;
	//double F;
	KDL::JntArray F;
	...
	//fk_pos_solver_->JntToCart(q_, x_);
        ...
        std::vector<double> obst_point_distances;
        
        //Let's collect the distances in a vector and find the index of min:
        for (size_t i = 0; i < obst_points_.size(); i++)
        {
        	double dist = sqrt(pow(obst_points_[i].p(0)-x_.p(0),2)+pow(obst_points_[i].p(1)-x_.p(1),2)+pow(obst_points_[i].p(2)-x_.p(2),2));
        	obst_point_distances.push_back(dist);
        }
        
        auto it = std::min_element(std::begin(obst_point_distances), std::end(obst_point_distances));
	int index_of_min = std::distance(std::begin(obst_point_distances), it);
	closest_point = obst_points_[index_of_min];
	
	dist_to_obst_tot_ = sqrt(pow(closest_point.p(0)-x_.p(0),2)+pow(closest_point.p(1)-x_.p(1),2)+pow(closest_point.p(2)-x_.p(2),2));
	    
        if (dist_to_obst_tot_ <= q_star)
        {
        	KDL::JntArray difference_vec;
        	difference_vec.data = Eigen::VectorXd::Zero(3);
        	//ROS_INFO("Ennen difference_vec alustusta");
        	
        	//ROS_INFO("Closest_point x y z: %f %f %f",closest_point.p(0), closest_point.p(1), closest_point.p(2));
        	
        	difference_vec(0)=x_.p(0)-closest_point.p(0);
        	difference_vec(1)=x_.p(1)-closest_point.p(1);
        	difference_vec(2)=x_.p(2)-closest_point.p(2);
        	//ROS_INFO("difference_vec alustuksen jälkeen");
        	...
        	//ROS_INFO("F * qdot_: %f", (double)F.data.dot(qdot_.data));
    ...
    void save_data()
    {
        ...
        /*ROS_INFO("x of the robot: %f  x of the obst: %f", (double)x_.p(0), (double)obstacle_.p(0));
        ROS_INFO("y of the robot: %f  y of the obst: %f", (double)x_.p(1), (double)obstacle_.p(1));
        ROS_INFO("z of the robot: %f  z of the obst: %f", (double)x_.p(2), (double)obstacle_.p(2));*/
  ...
  private:
    ...
    double obst_size_z;
    //KDL::Frame obstacle_;
    KDL::Frame obst_origin_;
    KDL::Frame obst_top_right_front_;
    KDL::Frame obst_top_left_front_;
    KDL::Frame obst_top_middle_front_;
    KDL::Frame obst_top_right_back_;
    KDL::Frame obst_top_left_back_;
    KDL::Frame obst_top_middle_back_;
    KDL::Frame obst_bottom_right_front_;
    KDL::Frame obst_bottom_left_front_;
    KDL::Frame obst_bottom_middle_front_;
    KDL::Frame obst_middle_right_front_;
    KDL::Frame obst_middle_left_front_;
    KDL::Frame obst_middle_middle_front_;
    KDL::Frame closest_point;
    std::vector<KDL::Frame> obst_points_;
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::Wall_obst_controller, controller_interface::ControllerBase)
